...
const AuthLayout = ({ children }) => {
  return (
    <div className='flex'>
      ...
      <div className='hidden md:block w-[40vw] h-screen bg-violet-50 bg-auth-bg-img bg-cover bg-center overflow-hidden relative p-8 bg-no-repeat'>
        <div className='w-48 h-48 rounded-[40px] bg-blue-600 absolute -top-7 -left-5'></div>
        <div className='w-48 h-56 rounded-[40px] border-[20px] border-blue-500 absolute top-[30%]'></div>
        <div className='w-48 h-48 rounded-[40px] bg-blue-500 absolute -bottom-7 -left-5'></div>
...
const StatsInfocard = ({ icon, label, value, color }) => {
  return (
    <div className="flex gap-6 bg-white rounded-xl shadow-md shadow-blue-400/10 border border-gray-200/50 z-10">
      <div 
        className={`w-12 h-12 flex items-center justify-center text-[26px] text-white ${color} rounded-full drop-shadow-xl`}
      >
        {icon}
